...
const (
accessTokenDuration = 24 _ time.Hour
refreshTokenDuration = 7 _ 24 \* time.Hour
)
...
func (s _courseService) CheckDeadlines() error {
deadlineThreshold := time.Now().Add(24 _ time.Hour)
var assignments []model.Assignment
if err := s.db.
Where("due*date BETWEEN ? AND ?", time.Now(), deadlineThreshold).
Preload("Course").
...
var enrollments []model.Enrollment
if err := s.db.Where("course*id = ?", assignment.CourseID).Find(&enrollments).Error; err != nil {
continue
}
for *, enrollment := range enrollments {
